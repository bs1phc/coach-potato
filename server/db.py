...
def replace_map_events(conn, match_id, puuid, events, source="timeline"):
    ...
    Two sources feed this table and they must not fight:
      * 'timeline' — Riot's match timeline, has positions, needs an API key.
      * 'ascent_log' — League's Live Client Data as captured in Ascent's logs;
    ...
    positions). `has_map_events` is only claimed by the timeline source — it is
