...
def test_review_queue_flags_block_game_with_no_notes(conn):
    ...
    assert row["game_creation_ms"] == BASE
...
def test_review_queue_excludes_block_game_with_notes(conn):
    add_match(conn, my_champ="Ahri", opp_champ="Zed", when=BASE, in_block=True)
    entry_id = conn.execute("SELECT id FROM block_games").fetchone()["id"]
    db.update_block_game(conn, entry_id, "wrote something down")
    assert stats.review_queue(conn, ME) == []


def test_review_queue_newest_first(conn):
    add_match(conn, my_champ="Garen", opp_champ="Darius", when=BASE, in_block=True)
    add_match(conn, my_champ="Yasuo", opp_champ="Riven", when=BASE + DAY_MS, in_block=True)
    rows = stats.review_queue(conn, ME)
    assert [(r["my_champion"], r["opp_champion"]) for r in rows] == [
        ("Yasuo", "Riven"), ("Garen", "Darius")]
